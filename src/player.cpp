...
void player::draw (BITMAP *temp, int tile_map_x, int tile_map_y) {
  if (!sliding) {
    if (jumping || canFall) {
      ...
      if (idle_timer <= 50 && characterDir == LEFT) {
        draw_sprite (temp, player_images[0], x - tile_map_x, y - tile_map_y);
      }

      if (idle_timer <= 50 && characterDir == RIGHT) {
        draw_sprite (temp, player_images[4], x - tile_map_x, y - tile_map_y);
      }

      if (idle_timer > 50 && characterDir == LEFT) {
        draw_sprite (temp, player_images[28], x - tile_map_x, y - tile_map_y);
      }

      if (idle_timer > 50 && characterDir == RIGHT) {
        draw_sprite (temp, player_images[29], x - tile_map_x, y - tile_map_y);
      }
    }
    else {
